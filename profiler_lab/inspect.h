...
static const char* address_to_function(pid_t pid, void* addr) {
  ...
  if(maps_file == NULL) {
    fprintf(stderr, "maps_file is NULL!!!!!!!\n");
    return NULL; 
  }
  ...
  if(!match_found) {
   fprintf(stderr, "match_found is not true!!!!\n");
   return NULL;
 }
  ...
  if(d == dwarf_map.end()) {
    ...
    if(fd == -1) {
      fprintf(stderr, "fd for open did not work!!!\n");
      return NULL;
    }
    ...
    if(!f.valid()) {
      close(fd);
      fprintf(stderr, "mmap_loader failed!!!\n");
      return NULL;
    }
    ...
    try {
      ...
      fprintf(stderr, "try statement didnt work!!!!\n");
      return NULL;
    }
  ...
  fprintf(stderr, "result from find_subprogram didnt work!!!\n");
